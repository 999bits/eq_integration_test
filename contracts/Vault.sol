...
contract Vault is SemiFungibleVault, ReentrancyGuard {
    ...
    constructor(
        ...
        int256 _strikePrice,
        address _controller
    ) SemiFungibleVault(_assetAddress, _name, _symbol) {
    ...
    function depositETH(
        uint256 id,
        address receiver
    ) external payable marketExists(id) epochHasNotStarted(id) nonReentrant {
    ...
    function totalAssets(
        uint256 _id
    ) public view override marketExists(_id) returns (uint256) {
    ...
    function calculateWithdrawalFeeValue(
        uint256 amount,
        uint256 _epoch
    ) public view returns (uint256 feeValue) {
    ...
    function setClaimTVL(
        uint256 id,
        uint256 claimTVL
    ) public onlyController marketExists(id) {
    ...
    */
    ...
    function previewWithdraw(
        uint256 id,
        uint256 assets
    ) public view override returns (uint256 entitledAmount) {
